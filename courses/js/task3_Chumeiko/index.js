...
const editedGoods = JSON.parse(JSON.stringify(goods));

const htmlGoods = document.getElementById('s-goods');
const btnSortPrice = document.getElementById('btn-price');
const btnSortRevievs = document.getElementById('btn-revievs');

const sortedByRevievs = goods.slice().sort(sortGoodsBy('ratingRevievs'));
const sortedByPrice = editGoodsPrice().sort(sortGoodsBy('price'));

window.addEventListener('load', outSortedGoods.bind(null, goods));
btnSortPrice.addEventListener('click', outSortedGoods.bind(null, sortedByPrice));
btnSortRevievs.addEventListener('click', outSortedGoods.bind(null, sortedByRevievs));

function outSortedGoods(goods) {
  let out = '';
  for (let item of goods) {
    out += `
        <div class="goods-item" id="goods-item">
          <span class="goods-item_name">Название: ${item.name}</span>
          <span class="goods-item_price">Цена: ${typeof item.price === 'object' ? item.price.newUan : item.price} ${(goods === sortedByPrice) ? 'грн' : ''}</span>
          <span class="goods-item_revievs">Отзывы: ${item.ratingRevievs}</span>
        </div>
    `
  }
  htmlGoods.innerHTML = out;
}

console.log(sortedByRevievs);
...
function editGoodsPrice() {
  ...
  for (let item of editedGoods) {
    if (parseInt(item.price)) {
      item.price = parseInt(item.price.replace(/[^0-9.]/gim, ""));
    }
    else {
      item.price = parseInt(item.price.newUan.replace(/[^0-9.]/gim, ""));
    }
  }
  return editedGoods;
}