let roles = ["Городничий","Аммос Федорович", "Артемий Филиппович","Лука Лукич", "Почтмейстер", "Бобчинский", "Добчинский", "Квартальный", "Частный пристав", "Анна Андреевна", "Марья Антоновна"];
...
createRoleText(roles, textLines);

function createRoleText(roles, textLines) {
  
  let out = '';
  textLines = textLines.split('\n');

  for (let role of roles) {
    out += `<div class="role">${role}: </div> <br>`;

    for (let i = 0; i < textLines.length; i++) {
      if ( textLines[i].startsWith(`${role}.`) ) {
        out += `<div class="text">${i+1}) ${textLines[i].slice(role.length + 1)}</div> <br>`;
      }
    }
    out += `<br>`;
  }

  document.getElementById('role-text-wrapper').insertAdjacentHTML('afterbegin', out);
}